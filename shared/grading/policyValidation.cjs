/**
 * Validates grading policy configuration (shared — used by API and UI).
 */
const { DEFAULT_GRADING_POLICY } = require('./policyDefaults.cjs');

function validateGradingPolicy(policy, { partial = false } = {}) {
  const errors = [];
  ...
  if (
    missing.mode &&
    !['count_as_zero', 'exclude_until_graded'].includes(missing.mode)
  ) {
  ...
  if (!partial && errors.length === 0 && !policy.version) {
  }

  return { valid: errors.length === 0, errors };
}

function sanitizeGradingPolicy(policy) {
...
function deepMergePolicy(target, source) {
  const out = { ...target };
