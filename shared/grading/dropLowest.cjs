...
/**
 * Returns assignment ids to exclude from group totals (lowest N scored items).
 */
function getDroppedAssignmentIds(groupAssignments, studentId, grades, submissions, dropLowest, groupName) {
  ...
  for (const assignment of groupAssignments) {
    ...
    scorable.push({ assignmentId, percent });
  }
