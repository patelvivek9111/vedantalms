 ...
 * ESM twin of policyApplication.cjs — keep in sync.
 */
import { resolvedPolicyFromSnapshot } from './policySnapshot.browser.mjs';

const VALID_APPLY_MODES = new Set(['retroactive_all', 'prospective_only', 'from_assignment']);

export function normalizeApplyMode(mode) {
  if (mode && VALID_APPLY_MODES.has(mode)) return mode;
  return 'retroactive_all';
}

export function buildPolicyApplication(coursePolicy = null, overrides = {}) {
...
export function getAssignmentGradedAt(submission, assignment) {
...
export function stripPolicyApplication(resolved) {
  if (!resolved || typeof resolved !== 'object') return resolved;
  const { policyApplication, ...rest } = resolved;
  return rest;
}

export function assignmentIsBeforeCutoff(app, assignmentId) {
...
export function resolveEffectivePolicyForAssignment(
  resolved,
  submission,
  assignment,
  gradeMode = 'current'
...
export function structuralPolicyPayload(payload = {}) {
  return Boolean(payload.groups || payload.gradeScale);
}

export function enrichResolvedForAssignmentOrder(resolved, assignments = []) {
...
export function buildPreviewPolicyApplication({
  applyMode,
  currentResolved,
  effectiveAt,
