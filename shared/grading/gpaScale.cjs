/**
 * GPA / scale mapping utilities.
 */
function getGpaPoints(letterGrade, gpaScale) {
  ...
  const row = mappings.find(
    (m) => m && String(m.letter).trim().toUpperCase() === normalized
  );
...
function percentToGpaScaleType(percent, gpaScale) {
