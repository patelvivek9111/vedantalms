...
import { isUnpublished, assignmentMaxPoints } from './groupActivation.mjs';
...
export function hasSubmissionScore(submission) {
  return (
...
export function releaseModeForAssignment(assignment) {
...
export function isScoreReleased(submission, assignment) {
...
export function resolveSubmissionGradeStatus({
  assignment,
  submission = null,
  grade = undefined,
  now = new Date(),
  ...
  if (hasSubmission) {
    ...
    // On-time submission awaiting grade — still "submitted" after due date (not missing).
...
export function gradebookCellFromStatus(statusResult, { grade, assignment }) {
...
export function mapWorkflowStateToGradeStatus(workflowState) {
  return WORKFLOW_STATE_TO_GRADE_STATUS[workflowState] || GRADE_STATUS.NOT_SUBMITTED;
}

export function mapGradeStatusToWorkflowState(statusResult, { assignment, submission, module, now = new Date() }) {
...
export function getGradeStatusLabel(status) {
  return GRADE_STATUS_LABELS[status] || status;
}

export function shouldShowStudentStatusBadge(status) {
  return [
