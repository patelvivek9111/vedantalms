    ...
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      {groups.length === 0 ? (
        <div className="text-center py-12">
          <div className="text-gray-500 dark:text-gray-400 text-lg mb-2">No groups found</div>
          <div className="text-gray-400 dark:text-gray-500">This group set doesn't have any groups yet.</div>
        </div>
      ) : (
            ...
            <div key={group._id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 border border-gray-100 dark:border-gray-700">
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1">{group.name}</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Group ID: {group.groupId}</p>
                  </div>
                </div>
                ...
                {group.leader && (
                  <div className="mb-4">
                    ...
                    <p className="text-sm text-gray-700 dark:text-gray-300">{group.leader.firstName} {group.leader.lastName}</p>
                  </div>
                )}
                
                <div className="flex gap-2 mt-4">
                  <button
                    className="flex-1 px-3 py-2 bg-blue-600 dark:bg-blue-500 text-white rounded-md hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors duration-200 text-sm font-medium"
                    onClick={() => navigate(`/groups/${group._id}`)}
                  >
                    View Group
                  </button>
                  <button
                    className="px-3 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200 text-sm"
